...
```
    ...
    <matplotlib.axes._subplots.AxesSubplot at 0x7fa4d11fe198>
...
```python
china_india.plot(kind='line', figsize=(14, 8))
...
```python
china_india.index = china_india.index.map(int) # let's change the index values to type integer for plotting
...
# function to get top 5 rows from the dataframe
df_5_top = df_can.nlargest(5, 'Total')

#Let's drop the unnecessary columns
df_5_top = df_5_top.drop(labels=['Continent', 'Region', 'DevName', 'Total'], axis=1)

#switch the columsn to rows
df_5_top = df_5_top.transpose()
...
```python
df_5_top.index = df_5_top.index.map(int) # let's change the index values to type integer for plotting
df_5_top.plot(kind='line', figsize=(14, 8))

plt.title('Immigration from top 5 countries by immigration')
plt.ylabel('Number of Immigrants')
plt.xlabel('Years')
