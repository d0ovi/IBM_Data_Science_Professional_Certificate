...
```python
df_canada.head()
# tip: You can specify the number of rows you'd like to see as follows: df_canada.head(10) 
```
...
# tip: The opposite of set is reset. So to reset the index, we can use df_canada.reset_index()
```
...
# [print (type(x)) for x in df_canada.columns.values] #<-- uncomment to check type of column headers
```
...
\\ inplace = True paramemter saves the changes to the original df_canada dataframe
